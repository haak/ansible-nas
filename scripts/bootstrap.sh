...
# if it is a laptop it cannot be a desktop or a server
if [[ "$laptop" =~ ^[yY]$ ]]; then
    desktop="n"
    server="n"
fi
...
if [[ "$laptop" =~ ^[nN]$ ]]; then
    ...
    # if it is a desktop it cannot be a server
    if [[ "$desktop" =~ ^[yY]$ ]]; then
        server="n"
    fi
fi
...
# use ip a command and grep for 192 to get ip address
IP=$(ip a | grep '192' | awk '{ print $2 }' | cut -d/ -f1)


echo "ip address: $IP"

#  ask user if hostname and ip are correct

# get username who ran the command:
...
# check if there is a inventory file with hostname in inventories/my-ansible-nas/host_vars
if [ -f "inventories/my-ansible-nas/host_vars/$BOX.yml" ]; then
    echo "inventory file exists"
else
    echo "inventory file does not exist"
    echo "creating inventory file"
    touch inventories/my-ansible-nas/host_vars/$BOX
    echo "ansible_host: $IP" >> inventories/my-ansible-nas/host_vars/$BOX
    echo "ansible_connection: ssh" >> inventories/my-ansible-nas/host_vars/$BOX
    echo "ansible_user: $USER" >> inventories/my-ansible-nas/host_vars/$BOX
fi
...
TODO: gnome dash to panel size
TODO: gnome startup applications

TODO: gnome traffic light buttons gnome
TODO: enable minimise and maximuse buttons in gnome tweaks under windows titlebars

TODO: gnome theme
TODO: gnome alt tab
TODO; gnome caps lock to esc


TODO: add phone to syncthing and make it introducer


TODO: other apps from aur


TODO: chezmoi https://github.com/hussainweb/ansible-role-chezmoi

TODO: atuin https://github.com/ellie/atuin#shell-plugin

TODO: java + maven archlinux
